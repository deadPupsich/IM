...
export default function IncidentRow({ incident, columns }: IncidentRowProps) {
  ...
  return (
    <>
      <div className="grid bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        <div className="flex items-stretch" style={{ gridColumn: '1 / -1' }}>
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            {columns.map((col) => (
              <div
                key={col.key}
                className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 border-r border-gray-200 dark:border-gray-700 last:border-r-0 flex-shrink-0 truncate"
